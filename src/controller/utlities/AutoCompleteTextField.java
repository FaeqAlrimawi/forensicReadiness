...
//    setEntriesFromSystemTypes();
    ...
    // entriesPopup.show(AutoCompleteTextArea.this, Side.TOP, position.getMaxX()
  ...
  public void autoComplete(TextField textArea, String text, String oldText) {
    ...
    this.pos = textArea.getCaretPosition();
...
//          .asList(openCloseBracketMap.keySet().toArray(new Integer [openCloseBracketMap.size()]));
...
//        // search for an index of open bracket that has no close and less than
...
//
//      // if current position is between the open and close then no autocomplete
    ...
    for (int i = 0; i < count; i++) {
      ...
      item.setOnAction(new EventHandler<ActionEvent>() {
        public void handle(ActionEvent actionEvent) {
          ...
          builder.append(text.substring(0, pos - subWord.length())).append(result)
              .append(text.substring(pos, text.length()));

          textArea.setText(builder.toString());
          textArea.positionCaret(pos + (result.length() - subWord.length()));
          entriesPopup.hide();
        }
      });
      menuItems.add(item);
